...
contract Policy is Controller, Proxy, PolicyFacetBase {
  ...
  ) Controller(_settings) Proxy() public
  {
    ...
    dataUint256["underwriterCommissionBP"] = _commmissionsBP[1];
    dataUint256["claimsAdminCommissionBP"] = _commmissionsBP[2];
    dataUint256["naymsCommissionBP"] = _commmissionsBP[3];

    // set roles, _stakeholders[0] is the treasury
    acl().assignRole(aclContext(), tx.origin, ROLE_POLICY_OWNER);
    ...
    if (_isRepOfEntity(tx.origin, _stakeholders[2])) {
    ...
    else if (_isRepOfEntity(tx.origin, _stakeholders[1])) {
